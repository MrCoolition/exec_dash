...
def test_login_with_auth0_blocks_flattened_auth_credentials_before_redirect(monkeypatch):
    monkeypatch.setattr(
        auth,
        "load_auth_config",
        lambda: {
    ...
    assert captured_errors
    assert "Failed Exchange: Unauthorized" in captured_errors[0]
    assert login_called["value"] is False
...
def test_login_with_auth0_blocks_control_characters_before_redirect(monkeypatch):
    monkeypatch.setattr(
        auth,
        "load_auth_config",
        lambda: {
    ...
    assert captured_errors
    assert "contains control characters" in captured_errors[0]
    assert login_called["value"] is False
